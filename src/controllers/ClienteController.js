...
async function atualizar(req, res) {
    const clienteAtualizado = await Cliente.findByIdAndUpdate(req.params.id, req.body,{ new: true })
        if (clienteAtualizado) {
            res.json(
                {
                    mensagem: "Cliente atualizado com sucesso", clienteAtualizado
                }
            )
        }   else {
            res.status(404).json({ mensagem: "Cliente não encontrado!"})
...
async function excluir(req, res) {
    const clienteExcluido = await Cliente.findByIdAndDelete(req.params.id)
        if (clienteExcluido) {
            res.json(
                {
                    mensagem: "Cliente excluido com sucesso!", clienteExcluido
                }
            )
        }   else {
            res.status(404).json({ mensagem: "Clinte não encontrado!"})
        }
}
